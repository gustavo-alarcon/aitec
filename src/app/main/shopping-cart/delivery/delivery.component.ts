...
export class DeliveryComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.filteredDepartamento$ = this.formGroup.get('departamento').valueChanges.pipe(
      startWith(''),
      map(value => {
        return this.places.filter(el=>value?el.departamento.toLowerCase().includes(value):true)
      })
    );
    ...
    this.filteredProvincia$ = this.formGroup.get('provincia').valueChanges.pipe(
      startWith(''),
      map(value => {
        return this.provincias.filter(el=>value?el.provincia.toLowerCase().includes(value):true)
      })
    );
    ...
    this.filteredDistrito$ = this.formGroup.get('distrito').valueChanges.pipe(
      ...
      map(value => {
        ...
        return this.distritos.filter(el=>value?el.distrito.toLowerCase().includes(value):true)
      })
    );
  }

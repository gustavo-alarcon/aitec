...
export class QuantityDivComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.quantity$ = this.dbs.orderObs$.pipe(
      map(order => {
        let index = order.findIndex(el => el['product']['sku'] == this.product['sku'])
        if (index >= 0) {
  ...
  changeQuantity(number){
    let index = this.dbs.order.findIndex(el => el['product']['sku'] == this.product['sku'])
    if (number == 0 || isNaN(number)) {
      this.dbs.order[index]['quantity'] = 1
    } else {
      if(number>=this.product['realStock']){
        this.dbs.order[index]['quantity'] = this.product['realStock']
        this.snackBar.open('Stock disponible del producto:' + this.product['realStock'], 'Aceptar', {
          duration: 6000
        })
      }else{
  ...
  onKeydown(event) {
    ...
    let permit = event.keyCode === 8 || event.keyCode === 46 || event.keyCode === 37 || event.keyCode === 39
